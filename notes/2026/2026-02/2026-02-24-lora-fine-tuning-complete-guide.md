...
### Exp4 High Capacity (r=32) Results

#### Training Loss
- Range: 2.58 → 1.65 (big drop then plateau)
- Rapid improvement in steps 10-50 (model learning fast)
- Plateau after step 60 (model saturated)

#### Eval Loss
- Range: 1.876 → 1.854 (steady decline)
- Smooth downward trend, no overfitting detected
- Only 2 eval points (eval every 50 steps, 129 total steps)

### How to Read Both Charts Together

- Both decreasing: Model is learning AND generalizing (GOOD)
- Train decreasing, Eval increasing: OVERFITTING (model memorizing)
- Both flat: Model stopped learning, need parameter changes
- Both increasing: Something is wrong with training

### Train vs Eval Loss Gap

Training loss (1.65) is lower than eval loss (1.854). Gap of ~0.2 is normal.
A large gap (e.g., train=0.5, eval=2.0) would indicate severe overfitting.

### Original vs Smoothed Lines

Each chart shows two lines:
- Light blue (original): Raw values, noisy due to batch variance
- Dark blue (smoothed): Moving average, shows the true trend clearly

### Images

![Training Loss](../../assets/2026-02-24/exp4_training_loss.png)
![Eval Loss](../../assets/2026-02-24/exp4_training_eval_loss.png)
