...
> 我的研究方向是利用 LLM Agent 自动化生成 fuzzing harness。核心项目是 **HarnessAgent**：针对 OSS-Fuzz 项目中的内部函数（非 API 函数），自动生成可编译、可运行、能有效提升覆盖率的 fuzz driver。系统基于 LangGraph 构建了一个工具增强的 Agent 流水线，包含 8 个代码分析工具、规则化的编译错误分类修复、以及一套检测 LLM "造假"行为的验证机制。在 243 个 OSS-Fuzz 目标函数上，三次尝试成功率达到 87%（C）和 81%（C++），比 SOTA 提升约 20%，并在 11 个真实项目中发现了 15 个新漏洞。
